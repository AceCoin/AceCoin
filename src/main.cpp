...
unsigned int nStakeTargetSpacing = 15;			        // 15 sec block spacing for PoS
unsigned int nWorkTargetSpacing = 90;                   // 90 sec block spacing for PoW
...
const string strMessageMagic = "AceCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	int nReduceFactor = nPoWHeight / 19200;
...
static const int64_t ifac1 = 99;		
static const int64_t ifac2 = 9;	
...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	...
	int random = generateMTRandom(seed, 67200);

	// printf(">> cseed = %s, random = %d\n", cseed, random);

	if(random > 48700 && random < 48711)		// 1/6720
	{
		return ifac1;
	}
	else if(random > 22900 && random < 22971)	// 1/960
	{
		return ifac2;
	}
...
static const int YEARLY_POS_BLOCK_COUNT = 2102400;

int64_t GetProofOfStakeReward(int64_t nCoinAge, const CBlockIndex* pindex)
{
    int64_t nRewardCoinYear = MAX_PROOF_OF_STAKE_STEADY;
	...
	{
		nSubsidy = 2.5 * nRewardCoinYear * nCoinAge / 365;
	}
	else 	
	{
	...
	// printf(">> PoS Created = %s, nSubsidy = %"PRId64", nCoinAge = %"PRId64", nRewardCoinYear = %"PRId64"\n", 
	//	FormatMoney(nSubsidy).c_str(), nSubsidy, nCoinAge, nRewardCoinYear);
	return nSubsidy;
}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AceCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "May 12, 2014: Ukraine's Donetsk region asking to join Russia";
        CTransaction txNew;
        txNew.nTime = 1399999576;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1399999576;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1465151;
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xe661ba6ee1a25a0313ae4e2818a498ddd25688e9433b7ba7ee91b82fd664f7c5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf0, 0xfb, 0xdb, 0xfd };
